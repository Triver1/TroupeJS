...
export class Agent {
  ...
  async sendToAgent(name: string, message: string): Promise<string | null> {
    const agent = globalRegistry.getAgent(name);

    if (!agent) {
      throw new AgentError(`Unknown agent "${name}"`);
    }

    const result = await AgentExecutor.respond(agent, message, {
      sender: this.name,
      target: agent.name,
      scope: "directed",
    });
